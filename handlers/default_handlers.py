...
from loader import dp, bot                          # импортируем диспатчер и бота
...
async def bot_help(message: Message):
    ...
    text2 = '\n'.join(text)
    # await bot.send_message(message.from_user.id, f"Команды бота:\n{text2}", reply_markup=kb_help)
    await bot.send_message(message.from_user.id, f"Команды бота в меню", reply_markup=kb_help)
